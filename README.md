...
* If MODS is specified as the value of `--key_datastream`, the DC datastream that is generated for each object will contain only the object's label and PID. If you want to DC datastream to be derived from the MODS.xml file (Islandora's default behavior), include the `--create_dc_from_mods` option in the `islandora_batch_with_derivs_preprocess` command (e.g., `islandora_batch_with_derivs_preprocess --key_datastream=MODS --create_dc_from_mods`).
